...
contract AttestManager is AccessControl, ReentrancyGuard {
    ...
    struct Project {
        ...
        string name;
        string description;
        uint256 totalEstimatedBudget;
        ProjectStatus status;
    }

    struct Agency {
        address wallet;
        string metadataURI; // Name, details
        bool isRegistered;
    }
    ...
    struct Phase {
        ...
        string[] proofLinks; // Can be IPFS hashes or URLs
        PhaseStatus status;
    }
    ...
    struct Proposal {
        ...
        address agency;
        string metadataURI;
        uint256 totalFunds; // Standard Public Funds
    ...
    IAttestVault public immutable vault;
    ...
    event PhaseApproved(uint256 indexed proposalId, uint256 indexed phaseIndex, uint256 amountReleased);
    ...
    function verifyAndReleasePhase(uint256 _proposalId) external onlyGovernment nonReentrant {
        ...
        // Release Funds
        if (currentPhase.allocatedAmount > 0) {
             vault.release(p.agency, currentPhase.allocatedAmount);
        }

        emit PhaseApproved(_proposalId, pIdx, currentPhase.allocatedAmount);
